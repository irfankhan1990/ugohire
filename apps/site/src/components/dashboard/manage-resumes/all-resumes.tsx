...
const AllResumes = () => {
  ...
  const handleClick = async () => {
    try {
        const response = await axios.get('/api/execPython', {
            params: {
                cmd: 'World'
            }
        });
  ...
  return (
    <section className="mb-6">
      ...
      <div className="pb-6 text-right">
        ...
        <button className="!py-3 px-8 bg-themePrimary rounded-lg shadow-themePrimary ml-4" onClick={handleClick}>
            <a className="text-white font-medium text-xxs">Resume Parser</a>
        </button>
      </div>
      <>
        {/* table start here */}
