...
export const JobsFilter = ({
  ...
  useEffect(() => {
    const CallRouter = async () => {
      if (
        ...
        await setValue("city", router.query.city);
        await setCity(router.query.city);
      }
  ...
  useEffect(() => {
    const CallRouter = async () => {
      if (
        ...
        (router.query.zipCode !== "") !== zipCode
      ) {
        await setValue("zipCode", router.query.zipCode);
        await setZipCode(router.query.zipCode);
      }
      if (!router.query.zipCode && zipCode !== "") {
        await setZipCode("");
        await setValue("zipCode", "");
      }
    };
    CallRouter();
  }, [router.query.zipCode]);
  ...
  /* --------- job title auto complete router query --------- */
  useEffect(() => {
    if (
      watch("jobTitle") !== "" ||
      jobTitle ||
  ...
  /* --------- location auto complete router query --------- */
  useEffect(() => {
    if (
      watch("location") !== "" ||
      location ||
  ...
  /* --------- city auto complete router query --------- */
  useEffect(() => {
    if (
      watch("city") !== "" ||
      city ||
...
/* --------- zipCode auto complete router query --------- */
useEffect(() => {
  if (router.query.zipCode !== watch("zipCode")) {
    const values = {
      jobTitle: watch("jobTitle"),
      location: watch("location"),
      city: watch("city"),
  ...
  const city_name = register("city");
  // const city_name = register("location");
  const zipCode_name = register("zipCode");
  // const zipCode_name = register("location");
          ...
          <form className="border-b border-gray">
            ...
            <div className="mb-4">
              <input
                ...
                {...city_name}
                onChange={(e) => {
                  city_name.onChange(e); // method from hook form register
                  setCurrentPage(0); // current page reset to 0
                  setCity(e.target.value); // your method
                }}
                onBlur={city_name.onBlur}
                ref={city_name.ref}
                placeholder="City"
              />
            </div>
            <div className="mb-4">
              <input
                ...
                {...zipCode_name}
                onChange={(e) => {
                  zipCode_name.onChange(e); // method from hook form register
                  setCurrentPage(0); // current page reset to 0
                  setLocation(e.target.value); // your method
                }}
                onBlur={zipCode_name.onBlur}
                ref={zipCode_name.ref}
...
export const CandidateFilter = ({
  ...
  useEffect(() => {
    if(zipCode !== "" || (router.query.zipCode && serviceName === "")){
      const delayDebounceFn = setTimeout(() => {
        const values = {
          professionalTitle: serviceName,
          category: category,
            ...
            <div className="mb-4">
              <input
                ...
                onChange={(e) => {
                  zipCode_name.onChange(e); // method from hook form register
                  setCurrentPage(0); // current page reset to 0
                  setZipCode(e.target.value); // your method
                }}
                onBlur={zipCode_name.onBlur}
                ref={zipCode_name.ref}
              />
